...
wsServer.on('request', function (request) {
    ...
    if (connection.remoteAddress == "::ffff:155.230.29.14") {
        Webos_conn = connection;
        console.log("webos 서버 connected");
        //Webos_conn.sendUTF("명훈이 받음?");
    }
    if (connection.remoteAddress == "::ffff:155.230.29.19") {
        Window_conn = connection;
        console.log("창문제어 서버 connected");
        //Webos_conn.sendUTF("명훈이 받음?");
    }
    ...
    connection.on('message', function (message) {
        if (message.type === 'utf8') {
            ...
            if (Webos_conn!="") {
               ...
               console.log("web으로 전송 : "+ message.utf8Data);
               Webos_conn.sendUTF(message.utf8Data);
            }
            if (Window_conn!="") {
                // connection.sendUTF("2명훈이 받음?");
                console.log("창문서버로 전송 : "+ message.utf8Data);
                Window_conn.sendUTF(message.utf8Data);
             }
            //Webos_conn.sendUTF(message.utf8Data)
        }
