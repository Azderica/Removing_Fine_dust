...
module ={} #아두이노 저장
module["a1"] = Arduino("a1",0.2)
module["a2"] = Arduino("a2",0.6)
module["a3"] = Arduino("a3",0.4)
module["a4"] = Arduino("a4",0.8)
module["camera"] = Cleaner("100,100,12345")

dust = find_dust_spot.FindDust(module["a1"].getValue(),module["a2"].getValue(),module["a3"].getValue(),module["a4"].getValue())
   ...
   def handle(self): # 클라이언트가 접속시 클라이언트 주소 출력
      ...
      if self.client_address[0]=='192.168.43.194':
         ...
         while True:
               ...
               #self.request.close()
               #break
            dust.setDust(module["a1"].getValue(),module["a2"].getValue(),module["a3"].getValue(),module["a4"].getValue())
            dust.setMap()
            #dust.setMiniMap()
            pos = dust.getMaxPos() 
            ...
            print(module["camera"])
            val1 = dust.getMoveRad(module["camera"].getX(),module["camera"].getY(),pos[0],pos[1],module["camera"].getRad())*(4000/36)
            
            distance = dust.getMoveDistance(module["camera"].getX(), module["camera"].getY(), pos[0], pos[1])
